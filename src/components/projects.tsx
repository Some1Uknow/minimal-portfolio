...
function MinorProjectCard({
  ...
  return (
    <InView
      ...
      className="group"
    >
      <article className="flex gap-3 p-3 rounded-lg bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-100 dark:border-zinc-800 hover:border-zinc-200 dark:hover:border-zinc-700 transition-all duration-300">
        {/* Thumbnail */}
        {project.image && (
          <div className="relative w-20 h-14 shrink-0 overflow-hidden rounded bg-zinc-100 dark:bg-zinc-800">
            <Image
              src={project.image}
              alt={`${project.title} screenshot`}
              fill
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[10px] text-zinc-600 dark:text-zinc-400 line-clamp-1 mb-1.5">
            {project.description}
          </p>
          <div className="flex flex-wrap gap-1">
            {project.tags.slice(0, 3).map((tag) => (
              <span
                key={tag}
                className="px-1 py-0.5 text-[9px] text-zinc-500 dark:text-zinc-500 bg-zinc-100 dark:bg-zinc-800 rounded"
              >
                {tag}
              </span>
            ))}
